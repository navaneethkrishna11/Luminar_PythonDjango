"""
URL configuration for authentication project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/4.2/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path
from app1 import views

urlpatterns = [
    path('admin/', admin.site.urls),
    path('', views.Home.as_view(), name='home'),
    path('login', views.UserLogin.as_view(), name='usrlogin'),
    path('signup', views.UserSignup.as_view(), name='usrsignup'),
    path('logout',views.UserLogout.as_view(),name='usrlogout'),
    path('teacherhome',views.TeacherHome.as_view(),name='teacherhome'),
    path('studenthome',views.StudentHome.as_view(),name='studenthome'),
    path('verify',views.OtpVerify.as_view(),name='verify'),
]
